<x-tomato-admin-layout>
    ...
    <x-slot:buttons>
        <x-tomato-admin-button :modal="true" :href="route('admin.orders-items.create')" type="link">
            {{trans('tomato-admin::global.crud.create-new')}} {{__('OrdersItem')}}
    ...
    <x-tomato-admin-row table type="bool" :value="$item->is_returned" />
                ...
                <x-splade-cell actions>
                    <div class="flex justify-start">
                        <x-tomato-admin-button success type="icon" title="{{trans('tomato-admin::global.crud.view')}}" modal :href="route('admin.orders-items.show', $item->id)">
                            <x-heroicon-s-eye class="h-6 w-6"/>
                        </x-tomato-admin-button>
                        <x-tomato-admin-button warning type="icon" title="{{trans('tomato-admin::global.crud.edit')}}" modal :href="route('admin.orders-items.edit', $item->id)">
                            <x-heroicon-s-pencil class="h-6 w-6"/>
                        </x-tomato-admin-button>
                        <x-tomato-admin-button danger type="icon" title="{{trans('tomato-admin::global.crud.delete')}}" :href="route('admin.orders-items.destroy', $item->id)"
